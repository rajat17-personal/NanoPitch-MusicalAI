...
def _note_pitch(pitch_post, onset, next_bound):
    """Predicted MIDI for a note spanning [onset, next_bound): take the argmax bin
    per frame over the span, then the MEDIAN bin (robust to onset/offset transients),
    and convert to MIDI. Returns NaN for an empty span."""
    lo, hi = int(onset), int(max(onset + 1, next_bound))
    span = pitch_post[lo:hi]
    if len(span) == 0:
        return float("nan")
    bins = span.argmax(axis=1)
    return float(_bin_to_midi(np.median(bins)))
